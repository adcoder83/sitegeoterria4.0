...
// Services/prestations de l’entreprise
const services = [
  {
    icon: "🧭",
    title: "Études de sol",
...
// Utilitaire pour (dés)activer le lien courant
...
function getAproposHTML() {
  return `
    <section class="page-section section-scroll">
      ...
      <div class="contact-multi-agency">
        <div class="agency-block">
          <h3>TOULON (83)</h3>
          <p>42 av Irène et Jean Frédéric Joliot Curie<br>83130 LA GARDE<br>Tel : 04 94 27 87 40</p>
        </div>
        <div class="agency-block">
          <h3>AIX-EN-PROVENCE (13)</h3>
          <p>Z.I Les Jalassières - 320 rue Topaze<br>13510 ÉGUILLES<br>Tel : 04 42 26 14 15</p>
        </div>
        <div class="agency-block">
          <h3>TOULOUSE (31)</h3>
          <p>24 bd Libre Échange<br>31650 - SAINT-ORENS-DE-GAMEVILLE<br>Tel : 05 62 27 12 82</p>
        </div>
      </div>
      <div style="margin:2em 0;">
        <img src="images/adresses-agences.png" alt="Adresses agences Géoterria" style="width:100%;max-width:800px;border-radius:5px;border:1px solid #e5e8ef;">
      </div>
    </section>
  `;
}
  ...
  return `
    <section class="page-section section-scroll">
      ...
      <div class="services-grid">
        ${services.map(s => `
          <div class="service-card" tabindex="0" aria-label="${s.title}">
    ...
    <section class="page-section section-scroll">
      ...
      <div class="team-grid">
        ${teamMembers.map(member => `
          <div class="team-card" data-key="${member.key}" tabindex="0" role="button" aria-label="Voir profil de ${member.nom}">
            <h3>${member.nom}</h3>
            <p>${member.role}</p>
          </div>
        `).join('')}
  ...
  return `
    <section class="page-section section-scroll">
      ...
      <div class="contact-info">
        ...
        <p><strong>Email :</strong> <a href="mailto:contact@geoterria.com">contact@geoterria.com</a></p>
      </div>
      <div class="contact-info" style="margin-top:1em;">
        <iframe src="https://www.openstreetmap.org/export/embed.html?bbox=6.0192%2C43.1243%2C6.0260%2C43.1271&amp;layer=mapnik&amp;marker=43.1257,6.0226"
          style="border:1px solid #e5e8ef; width:100%; max-width:450px; height:200px; border-radius:6px;" allowfullscreen="" loading="lazy"></iframe>
      </div>
    </section>
  `;
}

// Affichage d’une page dynamique
...
// Effet de "machine à écrire" sur la page d’accueil
...
// Animation cartes services/équipe
...
// ---------------------------
// Modale équipe, accessibilité et focus trap
// ---------------------------
...
function showMemberModal(memberKey) {
  ...
  if (!member || !modalContent || !modalBackdrop || !modalCard) return;
  ...
  modalCard.setAttribute('tabindex', '0');
  setTimeout(() => {
    modalCard.focus();
  }, 10);
  trapFocus(modalCard, modalBackdrop);
}
...
// Focus trap pour l’accessibilité modale
function trapFocus(modal, backdrop) {
  ...
  backdrop.addEventListener('keydown', function escClose(e) {
...
window.addEventListener('scroll', () => {
  if (mainContent && mainContent.classList.contains('parallax')) {
